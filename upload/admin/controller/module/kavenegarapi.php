...
class ControllerModuleKavenegarApi extends Controller {
	...
	public function install() {
	    ...
	    $this->model_module_kavenegarapi->install();
	    
    }

    public function uninstall() {
    	 $this->load->model('module/kavenegarapi');
	    $this->model_module_kavenegarapi->uninstall();
    }
    ...
    public function index() { 
		$this->load->language('module/kavenegarapi');
        ...
        $this->document->addStyle('view/stylesheet/kavenegarapi/kavenegarapi.css');
		$this->document->addScript('view/javascript/kavenegarapi/kavenegarapi.js');
		$this->document->addStyle('view/javascript/kavenegarapi/jquery/css/ui-lightness/jquery-ui.min.css');
		$this->document->addScript('view/javascript/kavenegarapi/jquery/js/jquery-ui.min.js');
		$this->document->addScript('view/javascript/kavenegarapi/smscounter.js');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) { 	
            ...
            $this->response->redirect(HTTP_SERVER.'index.php?route=module/kavenegarapi&store_id='.$this->request->post['store_id'] . '&token=' . $this->session->data['token']);
        }
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('module/kavenegarapi', 'token=' . $this->session->data['token'], 'SSL'),
        );
        ...
        $this->data['action']                 = $this->url->link('module/kavenegarapi', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['saveApiKey']             = $this->url->link('module/kavenegarapi/saveApiKey', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $this->response->setOutput($this->load->view('module/kavenegarapi.tpl', $this->data));
    }
			...
			switch ($this->request->post['to']) {
				case 'telephones':
					...
					foreach ($phones as $result) {
						$receptors[] = $this->model_module_kavenegarapi->formatNumber($result);
					}
					break;
				case 'newsletter':
					$customer_data = array(
					...
					$telephones_total = $this->model_module_kavenegarapi->getTotalCustomers($customer_data);
					$results = $this->model_module_kavenegarapi->getCustomers($customer_data);
					foreach ($results as $result) {
						$receptors[] = $this->model_module_kavenegarapi->formatNumber($result['telephone']);
					}
					break;
				case 'customer_all':
					$customer_data = array(
				...
				case 'customer':
					if (!empty($this->request->post['customer'])) {					
						foreach ($this->request->post['customer'] as $customer_id) {
							$customer_info = $this->model_module_kavenegarapi->getCustomer($customer_id);
							if ($customer_info) {
								$receptors[] = $this->model_module_kavenegarapi->formatNumber($customer_info['telephone']);
							}
						}
					}
					break;	
				case 'affiliate_all':
					...
					$telephones_total = $this->model_module_kavenegarapi->getTotalAffiliates($affiliate_data);		
					$results = $this->model_module_kavenegarapi->getAffiliates($affiliate_data);
					foreach ($results as $result) {
						$receptors[] = $this->model_module_kavenegarapi->formatNumber($result['telephone']);
					}						
					break;	
				case 'affiliate':
					if (!empty($this->request->post['affiliate'])) {					
						foreach ($this->request->post['affiliate'] as $affiliate_id) {
							$affiliate_info = $this->model_module_kavenegarapi->getAffiliate($affiliate_id);
							if ($affiliate_info) {
								$receptors[] = $this->model_module_kavenegarapi->formatNumber($affiliate_info['telephone']);
							}
						}
					}
					break;											
				case 'product':
					if (isset($this->request->post['product'])) {
						$results = $this->model_module_kavenegarapi->getTelephonesByProductsOrdered($this->request->post['product'], ($page - 1) * 10, 10);
						foreach ($results as $result) {
							$receptors[] = $this->model_module_kavenegarapi->formatNumber($result['telephone']);
						}
					}
